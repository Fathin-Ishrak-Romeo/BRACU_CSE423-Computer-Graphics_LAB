...
# Catcher settings (Restored previous width, height remains)
catcher_width = 80  # Restored width
catcher_height = 30
catcher_speed = 5
catcher_color = WHITE
...
# Function to draw left arrow button using mid-point line (restart button)
def draw_left_arrow(x, y, size, color):
    pygame.draw.line(screen, color, (x + size, y + size // 2), (x, y), 2)  # Top left diagonal
    pygame.draw.line(screen, color, (x, y), (x + size, y + size // 2), 2)  # Horizontal line
    pygame.draw.line(screen, color, (x + size, y + size // 2), (x + size, y), 2)  # Bottom right diagonal

# Function to draw play/pause button (Corrected size)
...
while True:
    ...
    # Draw catcher plate (restored width)
    ...
    # Draw UI buttons with correct sizes and fixed restart arrow shape
    draw_left_arrow(10, 10, 30, TEAL)  # Left arrow button (restart)
    draw_play_pause(180, 10, 30, AMBER, paused)  # Play/pause button (size fixed)
    draw_cross(360, 10, 30, RED)  # Cross button (exit)
